...
| 2 | 本地安装器别名 | — | v3.0 起已废弃——遗留 Shell 安装器已删除；别名只在 `install.py` 中注册（触点 1） |
...
!!! danger "install.py 是唯一的安装器"
    **统一 Python 安装器**（`install.py`）：使用 PEP 723 内联元数据，通过 `uv run` 自动引导，内置镜像回退，并通过 petfish-market 动态解析可选 pack。新 pack 必须在 `ALIASES` 字典中注册别名。

    四个遗留 Shell 安装器（`install.ps1`, `install.sh`, `remote-install.ps1`, `remote-install.sh`）已在 v3.0 中**删除**。任何仍在引用它们的文档或工具都是过时的——请一律使用 `install.py`。
...
PEtFiSh 只提供一个安装器：统一的 Python `install.py`。四个遗留 Shell 安装器（`install.ps1`, `install.sh`, `remote-install.ps1`, `remote-install.sh`）已在 v3.0 中删除。

| 脚本 | 类型 | 架构 | 状态 |
|---|---|---|---|
| `install.py` | 统一, Python (PEP 723) | 动态别名 + 市场解析 | **首选** |

所有逻辑变更都以 `install.py` 为目标。
