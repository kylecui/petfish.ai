...
- **v3.4.10**: Repository alignment. Synced the topic plugins in `lib/plugin/` and `.opencode/plugin/` to the pack copies, closing a delivery gap where only the pack copies carried the cache-first context rewrite and the three `topic-context-filter` fixes since 6f7223c. Removed two dead artifacts aimed at the installers deleted in v3.0 (`scripts/check_installer_parity.py`, `tests/test_migration_e2e.py`) and replaced the migration test with one that targets `install.py`. Fixed `verify_install.py`, which expected the shelved compaction plugin and required 4 registrations, so `/petfish verify` failed on every install. Corrected stale installer commands across the docs site, pack READMEs, and website; filled the v2.x/v3.x version-history gaps in the README and both changelogs; and baselined the pack version-drift gate on the highest released tag instead of `git describe`. Packs: fish-trail 1.4.0, petfish-companion-skill 1.9.3, research-skill-pack 1.0.2, doc-reader-skill 1.0.1.
...
- **v3.4.8**: P0 fix for a companion-gateway contract violation: `output.system` is a shared string[] that plugins may only `push` to, but the gateway used `+=`, poisoning every later plugin's `push` call and silently discarding its own injected content since release. One-line fix (`output.system.push()`) across 3 copies; added a 7th permanent release gate that statically scans all pack plugins for `output` field rebinding. Companion 1.9.2.
- **v3.4.7**: skills.sh and ClawHub integrated as first-class `fish-market` sources (13 total). skills.sh uses the official CLI search endpoint (`/api/search`); ClawHub is the OpenClaw registry (10.7K+ skills, documented `/v1/search`, 3000/min unauthenticated, includes Chinese skills). Companion 1.9.1.
- **v3.4.6**: MCP Official Registry dedupe now keys on plain `name`, since multiple remote entries for the same server share a display name.
- **v3.4.5**: MCP Official Registry results deduplicated by `name`+`url`, because its weak `q` filter returned duplicate entries.
- **v3.4.4**: `/petfish load` discovery reworked into a marketplaces-first ladder after three root causes pushed it into minute-long GitHub mining: 4 of 8 search sources had been dead since launch (`urllib.request.quote` AttributeError, silently swallowed per-source), Chinese queries against English indexes returned zero results with no retry hint, and there was no enforced "marketplace before mining" step. Added ClaudSkills (69K+ SKILL.md, Chinese-capable), PulseMCP, and the official MCP Registry. Companion 1.9.0 / toolchain 0.3.1.
- **v3.4.3**: Upgrade flow rewritten natural-language-first: the one-liner is the default entry and the agent runs all commands, so users stay in chat; Step 8's closing report now includes an install health matrix, current capabilities, per-platform restart guidance, and a post-restart conversational example.
- **v3.4.2**: Added `/petfish verify`, a one-command visual verification built on `verify_install.py`: a 10-item PASS/FAIL checklist (plugins, registration, MCP, vault self-test, gateway markers, L1 rule repair, index quality, version comparison, commands, course capability) with per-item remediation and a gate-compatible exit code. Companion 1.8.0.
- **v3.4.1**: Fixed `install.py` writing a legacy minimal `skill-index.json` that disabled gateway domain matching in user projects after v3.4.0. The installer now emits `domains` (from the shipped `catalog_query.py` single source), per-skill pack attribution, and best-effort market `available_packs`; verified live at 110 skills / 23 domains / 107 attributed / 15 market packs.
- **v3.4.0**: Completed all remaining phases of dynamic skill loading (P2+P3) and the courseware upgrade (P1+P2+P3): gap-aware discovery, `/petfish load <name|keyword> [--install]`, semantic repo mining with golden-repo benchmarks, market-side trigger derivation, vault usage tracking, a pedagogy reference layer plus `course-assessment-design`, an LLM-as-judge evals harness with CI gate, three offline teaching artifacts, and `course-delivery-review`. Companion 1.7.0 / toolchain 0.3.0 / course 1.5.0.

### v3.3 — Dynamic Skill Loading + Courseware

- **v3.3.0**: Dynamic skill loading P1 plus courseware P0. The new skill-vault MCP server delivers skill content as tool return values to bypass per-session skill discovery caching, exposing `vault_index`, `vault_fetch`, `vault_stage`, and `vault_install`; the Gateway injects a top-3 discovery block only when a capability gap has uninstalled candidates. Courseware P0 added machine-checked outline constraints, dual confirmation gates, and an authoring completion checklist. Companion 1.5.0 → 1.6.0 / course 1.3.2 → 1.4.0.

### v3.2 — Flexible Skills Management + Role-Based Profiles

- **v3.2.1**: Skill machinery foundation fixes (F1-F6): single-source triggers (`skill-index.json` gains a `domains` map, the gateway reads the index instead of a hardcoded table, and `gateway_classifiers.py` imports the catalog directly), `--skill NAME` granular install, a marketplace index `packs`/`skills` dual-key fix, project-aware `suggest` ranking, doc alignment, and a regression fix for calibrate eval phrases. Companion 1.5.0 / toolchain 0.2.0 / init 1.2.1.
...
### v2.5 → v2.0 — Context Filtering Hardening + Skill Suite

- **v2.5.2**: Fixed a TypeError on OpenCode messages lacking a `parts` array (system/metadata messages); null guards added throughout topic-context-filter, with `getMessageText()` returning an empty string, malformed messages scoring 999, and tool-pair propagation skipping them.
- **v2.5.1**: Self-healing topics: when `.petfish/fish-trail/` is deleted or missing, topic-context-filter auto-creates a default topic ("Current work session") and resumes normal operation instead of staying permanently offline.
- **v2.5.0**: Verbose JSONL logging to `.petfish/fish-trail/filter-debug.log` (guard activations, keep/remove counts and reasons, removed-message previews, active topic), console output via `debug: true`, 512KB auto-roll; includes the v2.4.8 and v2.4.9 system-directive and tool-result protections.
- **v2.4.9**: Tool results (file contents, command output) are never filtered and score 999 regardless of topic keyword match; previously they scored 0 and were removed, causing agents to lose context mid-task.
- **v2.4.8**: P0 fix preserving OpenCode system directives (TODO CONTINUATION, BACKGROUND TASK, `<system-reminder>`, `<!-- OMO_INTERNAL_INITIATOR`, mode markers) at score 999; they were previously removed for scoring 0, causing silent todo-hook failures and UnknownError exceptions.
- **v2.4.7**: Fixed #272: installer commands/agents loops now handle dict-format manifest entries, matching the v2.4.5 skills fix and preventing WindowsPath/dict TypeErrors on Windows.
- **v2.4.6**: Defensive manifest parsing for #269: `install.py` accepts both string and dict skills arrays; the typst-pdf-builder manifest was normalized and tagged v1.0.1.
- **v2.4.5**: Fixed 3 installer bugs (#264, #265, #253): dict-format skills in pack manifests (WindowsPath crash on Windows), `--pack all` now includes market packs in local mode, and `--check-updates` resolves pack repos from the market index.
- **v2.4.4**: Fixed remote `install.py` silently skipping all plugin installation by falling back to the core repo; remote installs now distribute every plugin.
- **v2.4.3**: Distributed `topic-context-filter.ts` (filters other-topic messages, roughly 30% token reduction in multi-topic sessions, with a 4-layer safety guard) and `fish-trail-compaction.ts` (topic-aware compaction prompt enrichment) to all projects.
- **v2.4.2**: Fixed 7 stale script paths in fish-brain/fish-market SKILL.md files; moved typst-pdf-builder to the public repo `kylecui/petfish-pack-typst-pdf-builder`; added external-repo support to `publish_pack.py`.
- **v2.4.1**: Fixed skill-publish's `publish_pack.py`, which hardcoded repo `kylecui/petfish.ai` and made externally published packs uninstallable; added `--repo`, `--path`, and `--repo-root` overrides.
- **v2.4.0**: De-AI skill suite: petfish-style pack grew from 1 to 3 skills with new `de-ai-detector` (hybrid quantitative + LLM AI-writing detection) and `style-extractor` (multi-dimensional style profiles from user samples); `petfish-style-rewriter` bumped to v5.0.0 with profile loading and post-rewrite re-detection.
- **v2.3.0**: Council Thinking v0.3.0: subagent-first execution, with the 5 advisor roles running as parallel oracle subagents in OpenCode and logical simulation as the fallback where `task()` is unavailable.
- **v2.1.0**: Fixed the #254 class of agents-rules drift: synced stale rule copies into pack directories, added MD5 drift detection as pre-release check #5, and added `verify_trace.py --coverage`.
- **v2.0.3**: drawio-radar-chart market compliance: created the missing v1.0.0 tag, changed the index ref from the `main` branch to the v1.0.0 tag, and updated the registry JSON to match.
- **v2.0.2**: Strengthened the skill-publish/skill-author pipeline against #249 recurrence: `validate_ref_exists()` blocks fake git tags before publishing, plus a mandatory pre-publish checklist and enforced `.opencode/skills/` layout.
- **v2.0.1**: Fixed #249: the drawio-radar-chart market entry pointed at a non-existent tag, lacked a manifest, and used a non-standard layout; ref changed to `main`, manifest added, layout restructured.
- **v2.0.0**: Issue triage: fixed #248 (fish-trail disk mode topic-detection gap; the agent now creates topics on major phase transitions), verified #223 (doc-reader remote install), and closed #170 (v4 compression ablation data recorded).
...
- **v1.9.5**: Documentation sync to v1.9.4: README and docs/zh brand bumped to v1.9, added v1.8.1 to v1.9.4 version history entries, synced the zh companion-gateway Step 0A/0B mode read, and updated the website timeline to the 6-step Gateway; fixed the `pre_release_check.py` market-ref false positive by downgrading separate-repo packs from FAIL to INFO.
...
> Note: v1.3.0 was never tagged or released as a separate version. Versioning went v1.1.0 → v1.4.0, and the work described below shipped as part of v1.4.0.
