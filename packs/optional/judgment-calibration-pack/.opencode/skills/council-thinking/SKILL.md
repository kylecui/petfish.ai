...
2. **并行启动 5 个独立 subagent**（`oracle` 类型，`run_in_background=true`），每个加载对应 agent prompt（`agents/critic.md` 等）：
   ...
   task(subagent_type="oracle", prompt=<outsider_prompt + 用户问题>, run_in_background=true)
   task(subagent_type="oracle", prompt=<executor_prompt + 用户问题>, run_in_background=true)
   ```
3. **等待 5 个 subagent 全部返回**（收集 `background_output`）。
4. **主代理作为 Arbiter** 执行 Step 3-5（交叉审查 → 删弱观点 → 综合结论），或启动第 6 个 Arbiter subagent 做最终综合。
5. 输出最终 Council 判断。

**优势**：每个顾问拥有独立推理上下文，避免视角污染和默认趋同——这是 Council 方法成立的核心条件。

### Path B: Logical Simulation（降级）

**仅当**运行环境不支持 subagent 调用时使用：

在同一轮回答中依次模拟 5 个角色的判断。遵循相同的输出结构和质量标准，但角色间可能存在隐性趋同。

### 模式选择规则

| 运行环境 | 默认路径 | 原因 |
|---|---|---|
| OpenCode（`task()` 可用） | **Path A — Subagent** | 原生支持并行 subagent 调度 |
| Claude Code（无 `task()`） | Path B — Logical | 不支持 subagent 编排 |
| Cursor / Copilot / Windsurf | Path B — Logical | 无 subagent 调度能力 |
| MCP-only 环境 | Path B — Logical | MCP tools 非 subagent |

> **判断标准**：如果当前 agent 可以调用 `task(subagent_type=..., run_in_background=true)` 且能收集异步结果，就用 Path A。否则用 Path B。
...
每个 Member 的独立 prompt 模板见 `agents/*.md`。Path A 下作为 subagent prompt 直接传入 `task()`；Path B 下作为角色定义在同一轮回答中模拟。
...
- 真实 subagent 编排实现（Path A 已支持 OpenCode 环境；其他环境待原生支持后自动启用）。
