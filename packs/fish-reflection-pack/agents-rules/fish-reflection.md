<!-- agents-rules/fish-reflection.md -->
...
## 自动检测行为（T1/T2 — Always-On）

以下行为在每次交互中自动执行，无需用户显式请求。

### T1: 用户纠正信号检测

扫描用户消息，检测以下纠正信号：

- 用户指出Agent输出有误："这不对"、"错了"、"不是这样"、"you're wrong"、"that's incorrect"
- 用户要求返工："重做"、"再来"、"改回去"、"redo"、"fix this"
- 用户纠正具体内容：提供正确答案或指出具体错误位置

**命中时行为**：先执行修正，修正完成后内联输出L1反思卡片（3-5行）。
...
**命中时行为**：暂停当前操作，输出L1反思卡片分析失败模式，调整策略后再继续。

### Session启动扫描

每次会话开始时，检查 `.opencode/reflections/` 目录是否存在：

- 若存在且包含反思记录，静默加载最近5条的prevention_rule作为上下文提醒
- 若不存在，不做任何操作

## 何时启用

- 用户指出Agent的输出有误并要求修正（T1用户纠正信号）
- 同一操作连续失败2次以上（T2重复失败信号）
- 用户显式请求反思或复盘（T3："reflect"、"反思"、"what went wrong"、"复盘"、"lessons learned"）
